...
# 
# The second thing to notice is that Ruby provides a method class_eval that takes a string and 
# evaluates it in the context of the current class, that is, the class from which you're calling 
# attr_accessor_with_history. This string will need to contain a method definition that implements 
...
# 
# Don't forget that the very first time the attribute receives a value, its history array will have 
# to be initialized.
# 
# Don't forget that instance variables are referred to as @bar within getters and setters, as 
# explained in Section 3.4 of ELLS.
# 
# Although the existing attr_accessor can handle multiple arguments (e.g. attr_accessor :foo, :bar), 
# your version just needs to handle a single argument. However, it should be able to track multiple 
# instance variables per class, with any legal class names or variable names, so it should work if 
# used this way:
# 
