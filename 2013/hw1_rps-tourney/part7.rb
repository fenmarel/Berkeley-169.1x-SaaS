...
# 
# Given two collections (of possibly different lengths), we want to get the Cartesian 
# product of the sequences. A Cartesian product is a sequence that enumerates every 
# possible pair from the two collections, where the pair consists of one element from 
# each collection. For example, the Cartesian product (denoted by ×) of the sequences 
...
# the ordering [ [:a,4], [:b,4], [:c,4], [:a,5], [:b,5], [:c,5] ] would be correct as 
# well.
# 
...
# Cartesian product a × b but [4,:a] is not. (Instead, [4,:a] is a member of the 
